...
class VocabularyRepository(dataDir: File) {
    private val vocabularyFile = File(dataDir, "vocabulary.json")
    ...
    suspend fun export(target: OutputTarget, words: List<SavedWord>) = withContext(Dispatchers.IO) {
        target.openStream().bufferedWriter(Charsets.UTF_8).use {
            it.write(csv(words))
        }
    }
