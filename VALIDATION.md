...
- **密度实测**：圣经整本 1.31–1.47（章密度 sd 0.06–0.12），魔戒 1.776。1.7 对圣经偏高 14–23%，DP 因此系统性偏好更长的中文节（漂移方向实测 Δ+1/+2 居多）。
- **全 DP 复跑**（同节/±1/覆盖）：每章密度 John **0.857/0.963/0.772**、Genesis **0.845/0.961/0.728**、Proverbs **0.769/0.946/0.673**（现状 0.649/0.599/0.465）；整本密度次之（0.836/0.828/0.709）。**句数归一反而更差**（0.535/0.518/0.305）——中文分句比英文多 20–50%，`(C_zh/S_zh)/(W_en/S_en)` 系统性低估 → **用字数，不要用句数**。
- **风险**：魔戒在基准读法下是 45 英文章对 29 中文章，章密度因误配波动到 0.40–10.31；scale 从 1.7 挪到整本 1.776 会让 **21.5%** 句对变化（1.71 已 5.2%，1.75 16.3%）→ 任何 scale 改动都要重跑金标准，大概率一轮判定。建议方案（整本密度 + 10% 死区 + 章级收缩 + 钳制，`VERSION`→6）与 A/B 两个选项写进 README，等审核。
