## 2026-09-08 语义代理 P3：候选词特异性加权——评估后不采用

**做法**：给 `SemanticProxyIndex` 加 `Config`（首释义 / 候选最短字数 / 候选上限 / 特异性加权）与变体对照台。两个验证工具每次打印 **共同子集 AUC**（= 所有变体都能评分的样本，否则各变体可评分集合不同、AUC 不可比）+ 全可评分 AUC。

**结果**（V6 对齐 + 第 8 轮 bless 后的当前基线）：

| 变体 | 圣经共同子集 n=2965 | 金标准共同子集 n=57 | 圣经覆盖 |
| --- | --- | --- | --- |
| `base`（候选上限 200、每词一票，**当前**） | 0.821 | 0.894 | 2,992 |
| `idf(200)`（特异性加权） | 0.810 | 0.878 | 2,992 |
| `idf_wide(5000)`（放宽上限 + 加权） | **0.834** | **0.899** | 3,121 |
| `primary`（仅首释义） | 0.806 | 0.870 | 3,023 |
| `primary_idf` | 0.798 | 0.841 | 3,023 |
| `idf_min2`（候选 ≥2 字） | 0.728 | 0.791 | 2,987 |

- 唯一稳定小胜的是 `idf_wide`（+1.3 / +0.5 点，约 **1 个标准误**），且多评分 4% 句对；但新增样本区分度低（金标准**全集** AUC 被稀释到 0.799），报警口径反而变差。
- 单释义、候选 ≥2 字明显变差——多义噪声和单字候选都带着真信号。
- **结论：收益在噪声量级，不采用**；默认评分保持「候选上限 200 + 每词一票」，变体对照台入库（`proxy` / `proxy-scale` 每次打印）。

**当前代理基线（V6 对齐 + 第 8 轮 bless 后）**：金标准 59 条够证据（ok/ok2 48 / bad 11）AUC **0.881**；圣经 2,992 条 AUC **0.821**（John 0.826 / Genesis 0.805 / Proverbs 0.816）；回归门基线 均值 0.339 / 中位 0.333 / p25 0.111 / p75 0.500。

| 项 | 证据 |
| --- | --- |
| 全量单测 | `./toolchain/build.sh testDebugUnitTest :shared:test` → app **387 用例 0 失败 0 跳过**、shared **286 用例 0 失败 0 跳过** |
| 工具 | `run-tool.sh proxy` / `proxy-scale` 均 `tests=1 failures=0` |
| 真机 | **未做**——只改测试工具与评估文档 |

## 2026-09-08 译本对齐 V6：长度归一化两级自适应（用户选 A；第 8 轮判定已完成）

**背景**：泛化集量化了单书过拟合，研究确认根因是全局常量 `ZH_CHARS_PER_EN_WORD = 1.7`，用户拍板按 **A（全量自适应，不加 10% 死区）** 落地。分支 `feat/adaptive-length-scale`。

**实现**（`TranslationAligner`，`VERSION` 5→6）：

- 段级密度 `s_p = clamp(章中文字符数 / 章英文词数)`，句级密度 `s_s = clamp(平均中文句长 / 平均英文句长)`，都钳制 `[0.9, 2.6]`；
- 整本字/词密度（只对全书求和、不依赖章配对）供章节对齐与兜底；**章内英文词数 < 200 退回整本密度**（短章密度是噪声，也让既有小样本单测行为不变）；全局 1.7 只在整本也拿不到特征时兜底；
- 段级 DP 用 `s_p`；句级 DP + V5 落盘门槛 + 1:N 合并门槛用 `s_s`；段落兜底的段级项用 `s_p`；
- 句数用**终止符近似计数**而非真分句（密度只要比例；实测指标持平或略好，省掉双倍分句开销）。

**生产实测**（`run-tool.sh generalization`）：

| 书 | V5 同节/±1/覆盖 | **V6 同节/±1/覆盖** |
| --- | --- | --- |
| John | 0.649 / 0.848 / 0.521 | **0.862 / 0.964 / 0.779** |
| Genesis | 0.599 / 0.816 / 0.408 | **0.846 / 0.960 / 0.743** |
| Proverbs | 0.465 / 0.773 / 0.270 | **0.769 / 0.945 / 0.728** |

泛化集三条下限按 V6 实测上调（同节 0.81/0.79/0.72，±1 0.91/0.91/0.89，覆盖 0.73/0.69/0.68）。

**性能**：benchmark 整本 950ms（V5 514ms，终止符扫描约 +430ms），点词命中率 **96.9% 不变**，<3s 护栏内。

**金标准（生产路径重放）**：句对 11,003（V5 11,017）；契约样本 ok/ok2 **65 条中 5 条变化**（s19/s27/s43/s59/s75，比预估 20–40 条小得多），bad/skip 12 条变化。

**第 8 轮人工判定（2026-09-08）**：用户逐条复核 17 条变化样本——5 条契约样本全部维持 ok/ok2（s19、s27 由 ok 降为 ok2），**s1 由 bad 升为 ok2**（新展示「他滿懷關切地說」对上了 `he cried, full of concern`），其余 11 条仍 bad。判定写入 `verdict-overrides.json` 第 8 轮 → fixture 并入台账（分布 ok 40 / ok2 26 / bad 34）→ bless（94 条批准）→ **重放转绿：契约样本 66 条全保持、bad/skip 28 条全保持、未定位 1 条**。

| 项 | 证据 |
| --- | --- |
| 单测 | `:shared:test` **286 用例 0 失败**（含合成语料 8 场景全 1.000、泛化集新下限、benchmark） |
| 金标准 | 第 8 轮判定完成并 bless：契约 **66/66 保持**、bad/skip 28/28 保持；台账 `verdict-history.json` 已更新（含 r8） |
| 真机 | **未做**——对齐是纯逻辑；attach 耗时需真机复测（PC 对齐 ~0.95s，旧基线 0.51s） |

## 2026-09-08 语义代理增强：证据门槛 + 圣经大规模验证 + 繁简归一

**背景**：上一轮代理 AUC 0.802 建立在 91 条人工判定上（bad 仅 27 条），置信区间很宽；t=0.2 报警精确率 0.55，主动采样 top-10 全是短对白（分数 0）。本轮按 P1（证据门槛 + Wilson 下界）与 P2（机械真值大规模验证）落地，P2 又暴露并修掉了「繁体语料 vs 简体候选」这个混淆项。说明见 `src/tools/alignment-eval/README.md` §6。

- **P1 证据门槛**（`SemanticProxyIndex`）：可查内容词 <3 的句对标 `scoreable=false`，不参与 AUC / 报警 / 排序（短对白不是「可疑」，是「没法评」）；新增 Wilson 95% 下界作报告列。金标准 91 条剔掉 31 条薄样本后 **AUC 0.802 → 0.901**（Wilson 0.877；实测原始命中率排序更好，故 `rank` 用命中率）。
- **P2 大规模验证**（新工具 `TranslationProxyBibleValidationTest`，`run-tool.sh proxy-scale`）：KJV × 和合本逐节真值，**2,268 条**可评分句对（同节 1327 / 错节 941）→ **AUC 0.833**（John 0.819 / Genesis 0.830 / Proverbs 0.836），跨文体一致。
- **繁简归一**：CC-CEDICT 候选是简体、评估语料（和合本、朱譯魔戒）是繁体；用对齐器同一张 `TraditionalSimplified` 表归一后，圣经 AUC 0.790 → **0.833**、同节样本均值 0.11 → 0.15。
- **结论**：排序能力跨文体稳定（0.83–0.90）；**绝对阈值不跨书**——圣经错节基率 41%，t=0.05 精确率 0.690 / 召回 0.777（无阈值到 0.70），魔戒 t=0.2 精确率 0.58 / 召回 0.92 → 报警必须按书内分布归一。回归门基线更新为均值 0.338 / 中位 0.333 / p25 0.125 / p75 0.500。**代理不得回灌对齐器代价函数**（循环论证）。

| 项 | 证据 |
| --- | --- |
| 全量单测 | `./toolchain/build.sh testDebugUnitTest :shared:test` → app **387 用例 0 失败 0 跳过**、shared **286 用例 0 失败 0 跳过** |
| 工具自检 | `run-tool.sh proxy` / `proxy-scale` 均 `tests=1 failures=0 errors=0 skipped=0`；判定卡无变化样本时正常早退 |
| 真机 | **未做**——本轮只改测试工具与评估文档，不触运行时 |

## 2026-09-08 译本对齐：长度归一化可行性研究 + 语料持久化 + 判定卡按可疑度排序

**背景**：泛化集暴露的整节漂移，根因指向全局常量 `TranslationAligner.ZH_CHARS_PER_EN_WORD = 1.7`。本轮做三件事：量化「章节密度自适应」的可行性（**不改生产代码**）、把评估语料从「文件在不在」升级为「清单 + sha256 可重建」、判定卡按语义代理分数排序。权威说明在 `src/tools/alignment-eval/README.md`（含「待审核」设计节）。

**1. 长度归一化可行性（结论：可行，收益大，但要一轮人工判定）**

- **方法**：把 aligner 的 `meaning = null` 路径一次性移植到 Python（`artifacts/length-scale-study/`，gitignored），**先用全局 1.7 复现 Kotlin 泛化集基线到小数点后三位**（John 0.649/0.848/0.521、Genesis 0.599/0.816/0.408、Proverbs 0.465/0.773/0.270；句对数 1087/1905/1064 一致），移植可信后只换 scale 重跑。
- **两种密度**：字/词密度（`C_zh/W_en`）圣经整本 1.31–1.47、魔戒 1.776；用户提出的句长密度（`W_en/S_en`，即平均英文句长）对应的句长比 `μ_zh/μ_en` 圣经 1.260/1.089/0.879、**魔戒 2.026**。恒等式 `C/W = (μ_zh/μ_en)×(S_zh/S_en)`：段级对齐要前者、句级对齐要后者，圣经两者差 15–50%，一个全局常量伺候不了两层。另：中英分句口径不一致（中文切 `；`、英文不切 `;`），统一口径后 `S_zh/S_en` 只有 1.00–1.10。
- **全 DP 复跑**（同节/±1/覆盖）：**两层方案（段=章密度、句=句长比）最优**——John **0.858/0.964/0.779**、Genesis **0.845/0.960/0.742**、Proverbs **0.768/0.945/0.729**（现状 0.649/0.599/0.465）；与「每章密度」精度持平但覆盖率更高（Proverbs 0.673→0.729、未定位 88→24）。**句长比两层都用则显著变差**（0.535/0.518/0.305）——段级需要 `C/W`。带句数修正的插值不稳定（Proverbs 0.636），不采用。
- **风险**：魔戒基准读法 45 英文章对 29 中文章，章密度因误配波动到 0.40–10.31；只改段级 21.5% 句对变化（1.71 已 5.2%），两层都改 **46.7%** → 任何 scale 改动都要重跑金标准、大概率一轮判定。建议方案（整本密度 + 10% 死区 + 章级收缩 + 钳制，`VERSION`→6）与 A/B 两个选项写进 README，等审核。

**2. 语料持久化**：`src/tools/alignment-eval/corpus-manifest.tsv` 钉住三个素材的来源/许可/字节/sha256；`fetch-corpus.sh [--check|--refresh|--only ids]` 负责下载 + 校验，两个旧 fetch 脚本变薄封装；`run-tool.sh full` 一条命令跑本地全量（replay→synthetic→generalization→proxy）。

**3. 判定卡按可疑度排序**：CC-CEDICT 索引与打分抽到 `SemanticProxyIndex.kt`（判定卡与代理工具共用同一实现），变化样本按代理分数升序出卡、元信息带分数；缺词典退回报告顺序并明确提示。

| 项 | 证据 |
| --- | --- |
| 全量单测 | `./toolchain/build.sh testDebugUnitTest :shared:test` → app **386 用例 0 失败 0 跳过**、shared **286 用例 0 失败 0 跳过** |
| 代理数字未变 | 抽取重构后 `run-tool.sh proxy` 输出逐字一致：AUC 0.802 / 0.756 / 0.434，整本分布 0.261/0.250/0.000/0.400 |
| 金标准未动 | `run-tool.sh replay`：契约样本 65/65 保持、与 `eval6.csv` 一致 81/94（与上一轮相同） |
| 判定卡排序 | 用合成的 5 条变化样本验证：按 0.00/0.00/0.50/0.67/1.00 升序、卡片元信息带分数（验证后已还原真实报告） |
| 语料校验 | `fetch-corpus.sh --check` 三个文件 sha256 全部通过 |
| 真机 | **未做**——本轮无 UI / 运行时行为改动 |

## 2026-09-08 译本对齐评估体系补齐：合成语料进 CI + 判定卡闭环 + 泛化集 + 语义代理验证

**背景**：金标准回归集（`d29ad7d`）落地后，按原五条建议的剩余部分继续：建议二的合成语料、建议三的语义代理、建议四的泛化集，以及建议一收尾的判定卡生成器。用户批准全做（A→B→C→D）。权威说明在 `src/tools/alignment-eval/README.md`。

**A 合成双语语料（`6c3d9d5`）**：`SyntheticAlignmentTruthTest` + `SyntheticBilingualCorpus`（`:shared`，CI 的 `:shared:test` 已包含）。每句嵌唯一编号、中英两侧同时出现 → 句对真值构造即知，「正确率」首次有机械断言。8 类场景（干净 / 漏译 / 段落合并拆分 / 标题行与纯标点 / 无编号靠词义锚点 / 繁体 / 编号窗口 / 2:1 句级合并）当前 precision、recall 全 1.000。

**B 判定卡生成器 + 第七轮入口（`108b06c`）**：`TranslationJudgmentCardTool` 把回归报告里的变化样本渲染成 HTML 判定卡（含一键收集 `sN=verdict;…` 的按钮）；`verdict-overrides.json` 入库，承接六轮之后的补充判定并写进台账 `rN`。第 7 轮 = 用户逐条复核 5 条 ok2（s57/s62/s69/s71/s74 从句子级变整段），**全部维持 ok2**。

**C 公有领域泛化集（`d122b4d`）**：KJV × 和合本（均公有领域、逐节对齐=免费真值）。实测**同节精度** John 0.649 / Genesis 0.599 / Proverbs 0.465，±1 节 0.773–0.848，节级覆盖 0.27–0.52——错误以整节漂移为主，怀疑「1.7 中文字/英文词」长度启发式不适配古典紧凑译本（DP 为凑长度整节错位）。**这是单书过拟合的量化证据**：以后重拟合门槛必须回泛化集复验。素材走 `fetch-generalization-corpus.sh`（本地、gitignored）。

**D 语义代理验证（`1574d48`）**：CC-CEDICT（独立于 ECDICT）内容词命中率对 91 条人工判定的 **AUC 0.802**（内部长度比 0.756、锚点 0.434 反向）。报警口径 t=0.2 精确率 0.55 / 召回 0.89 → 只够当粗筛。落地两用途：整本分布回归门（基线均值 0.261，漂移 >0.03 报警）+ 主动采样（排出 10 条「判对但存疑」）。未上句向量：LaBSE 约 1.8GB + Python/torch 与离线优先 / JVM 工具链冲突；替换点集中在 `scoreProxy` 一处。

| 项 | 证据 |
| --- | --- |
| 全量单测 | `./toolchain/build.sh testDebugUnitTest :shared:test` → app **386 用例 0 失败**、shared **286 用例 0 失败 0 跳过**（新测试在本机跑真实素材，CI 上素材缺失自动跳过） |
| 真机 | **未做**——本轮全部是评估/测试基础设施，不触 UI 与运行时行为 |
...
| 验证点 | 方法 | 结果 |
| --- | --- | --- |
| 来源入口「手动 AI 翻译…」 | UI 自动化点击 + 截图 | 第三项入口正常弹出对话框 ✅ |
| 覆盖进度初值 | 对话框文案 | 「已有译文 2/207 批」（此前在线试翻 2 批正确计入，hasCheckpoint 打标）✅ |
| 导出任务文件 | CreateDocument SAF 面板保存 | 1.12 MB JSON 落盘，批次/术语/指纹齐全；50 字符文件名截断不影响导入（按内容解析）✅ |
| agent 产出 | PC 端翻译卷名+第一章全 10 批（192 段，5.6 万字符），node 脚本与任务文件逐段对账 | 192/192 全覆盖、零缺失、零多余、编号=章内段落号 ✅ |
| 导入结果文件 | OpenMultipleDocuments 多选面板选中 Download 下的结果文件 | 11 批全部收下、0 拒绝（未弹拒绝对话框）✅ |
| 覆盖进度更新 | 重开对话框 | 「已有译文 13/207 批」（2+11，与预期严格一致）✅ |
| 部分覆盖语义 | 同上 | 未齐批不生成译本对照，书卡回到「加译本」态，符合断点续翻设计 ✅ |

**备注**：v1.6.2 首个 Release 资产因绕过 build.ps1 构建签错密钥库（真实用户 debug.keystore ≠ toolchain/guser），覆盖安装被 ColorOS 拒绝——已按 build.ps1 环境重构建并替换资产，keytool 指纹核验与 v1.6.1 一致（FF:9D…83:6F），线上资产下载比对逐字节一致；坑已记入 AGENTS.md。剩余 194 批需后续分次导入，齐批后自动离线生成译本对照（该自动生成路径由 Robolectric 端到端测试覆盖，真机待齐批后补验）。
...
**结论**：置信度能排序但 A 带仍 ~11% 错配（需独立信号），长度比可作强力拒斥信号；第三步方向 = 展示门 G2/G3 + ECDICT 词级锚点 + splitChinese 残渣修复（详见「归档-历史文档/译本对齐-质量画像-魔戒.md」§10）。
...
**主要结果**（详见「归档-历史文档/译本对齐-质量画像-魔戒.md」）：
...
## 2026-09-03 对齐器 V5（句对长度比硬门槛）：「长度明显不匹配」主诉修复

- **第 5 轮判定结论**（`alignment-eval5.html`，19 条重判）：合并错配率表面回升至 35%，其中 5 条 `.` 残渣为评估集垃圾（应用内无可点词不会发起查询，待从评估集剔除）、s50/s66 的 MISS 为评估工具章节桶错位（真实行为是 L3 命中错内容）；真实净变化：改善 4 条（s6/s43/s62/s96 拿到整段译文）。用户新主诉：长度明显不匹配的对照。
- **门槛拟合**（句对自身比例 × 判定交叉）：残 <0.45 的样本判定全 bad（3/3）；超 >1.9 中 ok 集最大 2.02。定门槛 **[0.45, 2.6]**，合并对（1:N，中文引文整句对多条短英文句是正常形态）放宽至 3.2。
- **实现**（`TranslationAligner.kt`，VERSION 4→5）：主 DP 与兜底 DP 的句对落盘前统一过长度比硬门槛，超限降级为段级条目（整段译文兜底）。置信度门槛管不住这类错对——锚点/词义加分能把长度比崩坏的对拉回 0.30 以上。
- **实现中发现并修正一处 V3 设计冲突**：合并门槛①（词义锚命中≥1）在 meaning=null 时恒 false，导致「引文整句对多条短英文句」这类必须合并的形态在无词典回退时被完全禁用（R1/R2 时代的旧测试因此转红）。修正：无词义证据时改用「局部 1:1 已无法解释长度关系（残或超）且合并后比例正常」放行。
- **整本重放 v6**：句对 11,395→11,016（句级 10,322 / 段级 694），**残缺句对清零**、长句残缺（en≥8 词比<0.35）64→0、比例>1.9 的句对 958→433（放行的 19 条超限合并对抽查均为引文合并真配对形态），平均置信度 0.900→0.912。主诉样本验证：s1 命中的句对自身比例 0.94（33 词合并句对）、s63 1.18、s75 0.84——「超/残」表象是评估工具用「查询段 vs 展示句」算的口径问题，非句对本身。
- **仍存问题（非长度问题）**：P2 样本（s78-s81）邻接兜底展示的中文仍是邻段内容（附录编年史段落）——邻接兜底的结构性错位，展示层应改为「无对照」而不是猜邻段；评估集 5 条 `.` 残渣待剔除。
- **JVM**：`testDebugUnitTest` **538 用例 0 失败**（新增 2 个：偏短/偏长门槛）。
- **待用户第 6 轮判定**（`alignment-eval6.html`，23 条变化）。

## 2026-09-03 V5 效果确认与评估口径修正
...
## 2026-09-06 桌面迁移 M2 刀1/刀2：AppContext 抽象 + ReviewMode/ReviewScheduler/DictionaryRepository 入 :shared

- **刀1（`5018f89`）**：新增 `com.linguareader.shared.app` 的 `AppContext`（当前仅 `prefs(name)`，按刀的实际消费逐成员扩展）与 `PreferencesStore`（getString/putString 最小面）；`ReviewMode`/`ReviewPace`/`ReviewReminders`/`ReviewScheduler` 迁入 `com.linguareader.shared.data`（`git show HEAD:<旧路径>` 逐行 diff 证明仅包名/labelRes→SharedString/PreferencesStore 签名三类预期差异），随迁 `ReviewModeTest` 11 条。Android 侧新增 `SharedPreferencesStore` 适配器 + `asPreferencesStore()`；`SharedString` 新增复习节奏 7 符号（穷举 when 编译强制补映射）；UI 消费点 `ReviewUi.kt` 2 处、`ReaderScreen.kt` 1 处改 `.resolve()`；`fromPreferences`/`write` 的 5 个调用点（AppViewModel/BootRescheduleReceiver/ReviewReminder/androidTest）全部改传适配器。**测试守恒 503+39=542**。门禁期间 `:shared:test` 抓到重写时 `ReviewReminders.toJson()` 丢 `toolbarBadge` 字段的手滑——守门测试有效。
...
## 2026-09-13 第四轮审查收口：21 条「未给建议项」验收口径落地

**范围**：`审查报告-第四轮-未给建议项-验收口径草案.md` 的 21 条待给口径（逐条实现/核实到「草案状态列无`待做`」），外加 `README.md` 版本句订正与第四轮产出的归档入库。

### 本次交付

| 项 | 内容 | 提交 |
| --- | --- | --- |
| R1 引擎身份 | 缓存键纳入 `serverModel` / MiMo 风格指令（换模型/改风格不再命中旧音频） | `0685482` |
| M6 缓存配额 | 整书缓存路径只保「最近写入的那一章」，单本超上限可淘汰（原先保护整本 ⇒ 配额失效） | `a3ae2d8` |
| R2 主题语义色 | 新增 `goldOnPaper` 供文字使用（浅色 paper 5.67:1），品牌金仅留图形 | `28a0092` |
| 5-10 主题入口 | 抽出公共 `ReaderThemePicker` + `storeReaderTheme`，接入书架「书架外观」弹层（点击深度 4→≤2） | `0d0b489` |
| 6-10 包内交叉校验 | 安装时断言 `manifest.engineTag/voice` 与载荷目录段 `TtsCacheKey.segmentDir` 一致，不一致拒装 | `2a83a0a` |
| 4-5 断词连字符 | `textToXhtml` 合并「字母-换行-小写字母」的排印断词（`exam- ple` → `example`），保留 `C-3PO`/`--` | `6b53bf9` |
| 4-7 TXT 标题 | 章标题在正文顶部生成 `<h1>`（数量 == 章节数）；`BLOCK_SELECTOR` 本就含 h1 故无需 bump | `ec1270b` |
| 6-14 / 7-13 占用口径 | 资源包页总数改取 `packs.totalBytes()`（扫盘实测），与存储页同源 + UI 标注「磁盘实测」 | `6d3edff` |
| 6-8 包加载校大小 | `AudioPackSource.resolve` 在清单声明了该文件时校验实际大小，截断文件不再当命中 | `7d45247` |
| 7-10 克隆音色过滤 | 新增纯函数 `usableCloneVoices`，样本缺失的音色不进音色库（避免分到必然失败的音色） | `f060c71` |
| 7-12 拒装降级包 | 装更低版本在提取前即拒并给出原因；版本比较按点分段数字（`1.10.0 > 1.9.0`） | `d296a28` |
| 7-4 登记表原子写 | 回退分支改为「重建 tmp 再 rename」；写失败不更新内存缓存（**重试分支无专门单测**，已注明） | `2b31dff` |
| 6-15 入口文案消歧 | 实测资源包 UI 本无「下载」字样；按钮「安装资源包」→「导入资源包」以消除在线下载预期 | `c5fd478`/`af9e8e2` |
| 7-8 dir 交叉校验 | `PackRegistry.parse` 校验 `dir == PackPaths.directoryFor(type,packId,version)`，漂移按登记表损坏处理 | `2664346` |
| 1-4 段级兜底粒度 | **经核实已满足**（UI 已标「段级（未定位到句）」、阈值 0.30、统计只计 SENTENCE），零代码改动 | `d63f934` |
| 2-4 句数口径（口径 A） | 英文侧改真实分句（偏差 1.65%→0），中文侧近似记为**已知例外**；连带 bless 3 条金标准样本 | `a2a9ce6`/`e8a4706` |
| 6-13 缓存淘汰 | 新增 `.accessed` 访问标记（1h 写放大护栏），`trimTo` 按 `lastAccessed` 排序（真 LRU）；`clearAll` 按删除结果统计 | `66befda` |
| 6-6 合成重试 + 降级提示 | 重试 1 次（退避 1s≤2s）；`TtsPlaybackState` 增 `degradedReason`，引擎两处上报、恢复时清空，`ListeningBar` 出可见提示 | `ca74394`/`ad7cb05`/`30d62ca` |
| 7-9 残留可见可清 | `PackRepository.residuals()` 推导式识别未登记目录/`.tmp` 残留 + 包列表页列出与清理入口 | `fea90bf` |
| 文档 | `README.md` 版本句订正（1.4.0/9 → 1.7.0/16 + targetSdk 35）；第四轮抽取报告补「指代约定（写法 A）」；归档路径订正；`归档-历史文档/`、`审查报告-第四轮.md`、`审查附件-第四轮/`、`议题整理/` 入库 | `415b78d`/`a6b8ea4`/`8517380`/`15ebb63`/`c3365b0`/`0b0c3c2` |

### 验证

- 单测（`./toolchain/build.sh testDebugUnitTest :shared:test`，实测）：`:app` **435 例** / `:shared` **373 例**，0 失败 0 错误。
- 金标准 bless：`TranslationGoldenReplayTest` 3 条 `ok2` 契约样本变化（s1/s27 段级兜底→句级定位、s75 段内归属更贴合），按授权执行 `bless.flag` 流程后批准 94 条。**注意**：`artifacts/alignment-eval/golden-samples.json` 位于被 gitignore 的 `artifacts/`，该 bless **只在本机生效、不随仓库分发**。
- 反向验证（失败即回退，均已回退）：2-4 口径①「中文也精确」会使 Proverbs 节级覆盖率掉到 0.674 < 下限 0.68，已回退；R4 对齐提速因测量被 Gradle 缓存污染、无法证实提速，已整体回退。
- 未验：6-6 的**真机**提示截图（执行时 `adb devices` 无设备，故只做了「原因→文案」纯函数映射的 JVM 护栏 `DegradedHintTest`，渲染层需设备）；5-10 / 7-9 的界面需真机目视；`7-4` 的 rename 重试分支未加专门单测（需模拟 `renameTo` 失败）。
- 已知例外：`2-4` 中文侧近似偏差约 50%（口径 A 明确接受，理由见条目）。
