## 2026-09-08 译本对齐 V6：长度归一化两级自适应（用户选 A；待第 8 轮判定后合入 main）
...
**金标准（生产路径重放）**：句对 11,003（V5 11,017）；契约样本 ok/ok2 **65 条中 5 条变化**（s19/s27/s43/s59/s75，比预估 20–40 条小得多），bad/skip 12 条变化；判定卡已生成（17 张，`artifacts/alignment-eval/judgment-cards.html`）。

| 项 | 证据 |
| --- | --- |
| 单测 | `:shared:test` **286 用例 0 失败**（含合成语料 8 场景全 1.000、泛化集新下限、benchmark） |
| 待办 | **第 8 轮人工判定**：17 张判定卡 → `verdict-overrides.json` → fixture → bless → 重放转绿 → 合入 main |
