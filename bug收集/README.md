...
| 编号 | 标题 | 严重程度 | 状态 | 关键改动 |
| --- | --- | --- | --- | --- |
| [BUG-035](./BUG-035-进入滑动模式先置位后取值导致跳回章首.md) | 进入滑动模式先置位后取值，进度归零跳章首 | 🟠 中等 | 待修复 | `enterScrollMode` 先取 currentRatio() 再置 scrollMode=true |
| [BUG-036](./BUG-036-末页哨兵被恢复判据排除导致回翻跳上一章开头.md) | `Int.MAX_VALUE` 末页哨兵被恢复判据排除，回翻跳上一章开头 | 🟠 中等 | 待修复 | 显式 LAST_PAGE 哨兵，`updateMetrics` 单独处理 |
...
| 编号 | 标题 | 严重程度 | 状态 | 关键改动 |
| --- | --- | --- | --- | --- |
| [BUG-027](./BUG-027-生词高亮无词边界误标子串-修复未落地回归.md) | 生词高亮无词边界误标子串（BUG-021 修复未落地） | 🟠 中等 | 待修复 | JS 侧补词边界；先查 git log -S'isWordChar' 是否被合并覆盖 |
| [BUG-028](./BUG-028-生词按原型存储导致变形词与短语无划线.md) | 生词按原型/短语存储，变形词与跨节点短语无划线 | 🟠 中等 | 待修复 | 下发 surfaceForms；`SavedWord` 加字段（向后兼容） |
| [BUG-029](./次要候选/BUG-029-生词高亮用存储词改写正文大小写.md) | 生词高亮用存储词替换原文，篡改正文大小写 | 🟡 轻微 | 待修复 | 回填用原文切片而非存储词 |
...
| [BUG-034](./次要候选/BUG-034-手动翻页无条件把朗读拉回本页首句.md) | 听书时手动翻页无条件把朗读拉回本页首句 | 🟡 轻微 | 待决策 | 仅当朗读句离开视口才重定位（需产品定调） |
...
| BUG-027 | 生词高亮无词边界误标子串（BUG-021 修复未落地） | 🟠 中等 | 待修复 | 批次 5.4 |
| BUG-028 | 生词按原型/短语存储，变形词与跨节点短语无划线 | 🟠 中等 | 待修复 | 批次 5.4 |
| BUG-029 | 生词高亮用存储词替换原文，篡改正文大小写 | 🟡 轻微 | 待修复 | 批次 5.4 |
...
| BUG-034 | 听书时手动翻页无条件把朗读拉回本页首句 | 🟡 轻微 | 待决策 | 批次 5.6 |
| BUG-035 | 进入滑动模式先置位后取值，进度归零跳章首 | 🟠 中等 | 待修复 | 批次 5.1 |
| BUG-036 | `Int.MAX_VALUE` 末页哨兵被恢复判据排除，回翻跳上一章开头 | 🟠 中等 | 待修复 | 批次 5.1 |
