...
> ## ⚠️ 状态列的来源与可信度（2026-08-29 追记）
>
> 本目录 31 篇文档由 `a7bd7c0`（2026-08-23，「收录 bug收集/ 缺陷文档 31 篇……自
> `legacy/remote-main-20260820` 移植」）**一次性整批导入，该提交只含 .md、不含任何代码**。
> 也就是说：**每一行「已修复」记录的都是 legacy 线的状态，不是 main 的状态。**
>
> 而把 legacy 修复搬到 main 的是另一次合并 `595af36`（「legacy 线 **26 项**缺陷修复移植」）
> ——**31 篇文档 vs 自称 26 项移植**，两个数字对不上，中间的差额没有台账。
>
> 已实证的漏网一例：**BUG-021**（生词高亮词边界）修复只在 legacy 的 `b784acd`，
> port 分支从未带上，main 的 `ReaderScripts.kt:747` 至今是裸 `indexOf`。
>
> 因此：**下表的「已修复」不能直接当作 main 的事实**。逐条对照 main 实际代码的全量核验
> 尚未做；引用本表前，请先 grep main 的对应文件确认。已核验过的条目会在行内注明。
...
| 编号 | 标题 | 严重程度 | 状态 | 关键改动 |
| --- | --- | --- | --- | --- |
| [BUG-019](./次要候选/BUG-019-targetIndex闭区间包含排他边界off-by-one.md) | `targetIndex` 用闭区间包含排他边界 `endExclusive`（off-by-one） | 🟡 轻微 | 已修复 ✅ main 已核验（`4cfc886`） | 改为 `in it.start until it.endExclusive` |
| [BUG-021](./次要候选/BUG-021-生词高亮无词边界误标子串.md) | 生词高亮无词边界，短词被误标进不相关单词 | 🟡 轻微 | ❌ **未修复（main）** | 修复只在 legacy `b784acd`，`595af36` 移植时遗漏；main `ReaderScripts.kt:747` 仍是裸 `indexOf` |
| [OBS-01](./次要候选/观察项-未定级.md) | 本地语境把句首大写普通词混入专名/术语档案 | ⚪ 未定级 | 已修复 ✅ main 已核验（`c931d06`） | 区分句首大写与句中大写，或加功能词黑名单 |

> 本族状态已于 2026-08-29 逐条对照 main 代码核验：BUG-019 / BUG-020 / OBS-01 三项修复
> 在 main 中真实存在，**仅 BUG-021 未落地**。取证过程见
> [BUG-021 文档「状态订正」节](./次要候选/BUG-021-生词高亮无词边界误标子串.md)。
...
- ⚠️ 追记（2026-08-29）：**本批次的验收全部发生在 legacy 线**（`legacy/remote-main-20260820`），
  上述测试结论与「已修复」状态描述的是 legacy 的代码，不代表 main。main 侧的对应移植
  由 `595af36` 完成且**存在遗漏**（已实证：BUG-021）。详见文首「状态列的来源与可信度」。

---

## 编号速查表

> 状态列同样源自 legacy 线整批导入（见文首告示）。除 BUG-019 / BUG-020 / BUG-021 / OBS-01
> 已于 2026-08-29 对照 main 核验外，其余各行**未经 main 代码验证**。
...
| BUG-021 | 生词高亮无词边界，短词被误标进不相关单词 | 🟡 轻微 | ❌ **未修复（main）** | 批次 2.3 |
...
- **词边界/术语/高亮族**：BUG-019、BUG-020、BUG-021、OBS-01
  ...
  - ⚠️ 该族中 Kotlin 侧三项（BUG-019/020/OBS-01）已在 main 落地，**唯独 JS 侧的 BUG-021 没有**
    ——两侧边界规则至今仍不一致，正是本族当初想消除的问题。
