...
# 可选鉴权：设置 AUTH_TOKEN 后，/voices、/v1/models、/v1/audio/speech 均要求
# `Authorization: Bearer <token>`。留空 = 不鉴权（向后兼容，但启动时会提示风险）。
...
def _require_auth():
    if _authorized():
        return None
    return jsonify({"error": "未授权或 Token 错误"}), 401
...
def main():
    ...
    print("=" * 62)
    if not AUTH_TOKEN:
        print("[警告] 未设置 AUTH_TOKEN：本服务未鉴权，任一局域网设备都可调用合成接口。")
        print("[建议] 仅在可信网络使用；如需鉴权请设置环境变量 AUTH_TOKEN=...")
