...
4. 同一默认值散落在代码多处（默认模型名在数据类默认值 / 存储层 load 回退 / save 回退三处各写一份，改名只改一处）→ `code-and-verification.md` 增补「默认值与标识符单点定义」；并记下「连通性测试通过 ≠ 标识符有效」。
5. 用捕获输出的包装脚本跑测试，构建守护进程持有重定向句柄导致外层假挂、作业状态骗人 → `code-and-verification.md` 增补「判据是产物，不是作业状态」，`AGENTS.template.md` 验证纪律同步加铁律。
