...
6. **派生数据的版本闸门「只写不读」**——算法/规则改进落进了代码，却因为存量数据带的版本号从来没被比较过，对老用户永远不生效，且没有任何报错 → `code-and-verification.md` 增补「派生数据：版本闸门必须被读」（含「闸门要配显式重建入口」与「同一批改动合并成一次 bump」）。
7. **契约基线没建立时，回归测试会以「缺基线」全红**，外观与真回归无法区分 → 同上节增补「先建基线再改代码」；同时源库实测出**比例型断言在语义变更后变成不可达目标**，松弛阈值又会掩盖真实错配 → 增补「期望值要显式，不要用比例阈值兜」。
8. **两个口径的数字互相矛盾、而旧口径的素材已丢失**（源库实测差 12 条、无法复现也无法对账）→ `memory-maintenance.md` 增补「不可复现的历史数值要退役，不要并列」：记录数值时注明来源环境与可再生性，矛盾时只留一个权威口径。
