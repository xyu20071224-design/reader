...
1. `align()` 先算整本 `s_book = ΣC_zh / ΣW_en` 与 `s_book_sent = μ_zh/μ_en`（都不依赖
   章配对，O(text)）；
2. 若 `|s_book/1.7 − 1| ≤ 0.10` → 整本沿用 1.7（魔戒落在带内 → 零 churn）；
3. 否则逐章 `s_c = (C_zh + α·s_book)/(W_en + α)`、`s_c_sent = (μ_zh·S_en + α·s_book_sent)/(W_en + α)`（α≈200 词），各自钳制 `[0.9, 2.6]`；
4. 段级 DP 用 `s_c`，句级 DP + V5 落盘门槛 + 1:N 合并门槛用 `s_c_sent`，`VERSION → 6`；
5. 验收：泛化集三书同节 ≥0.84 / ±1 ≥0.95 / 覆盖 ≥0.72，金标准变化样本跑一轮判定卡，
   benchmark 耗时 <3s 不退化。

两个选项：**A（推荐）全量自适应**——不要第 2 步死区，质量最高，代价是魔戒基线
约 20–40 条样本变化（一轮判定）；**B 零 churn**——保留 10% 死区，圣经三书照常受益
