...
错误以 Δ±1 节为主（整节漂移）。根因就是长度比启发式「1.7 中文字 / 英文词」不适配
和合本的紧凑用词，DP 为凑长度而整节错位。**这是单书过拟合的量化证据**，直接催生了
下面的 V6 两级自适应（同节精度升到 0.86 / 0.85 / 0.77）。测试对三本书各设
「同节 / ±1 节 / 覆盖率」三条下限（按 V6 实测留约 5 个点余量），红了说明泛化退化。
...
bash src/tools/alignment-eval/run-tool.sh proxy                # 金标准人工判定（89 条可评分）
bash src/tools/alignment-eval/run-tool.sh proxy-scale          # 圣经 ~3,000 条机械真值 + P3 变体选型台
```

**2026-09-08 三项增强（P1 证据门槛 / 繁简归一 / P2 大规模验证）**：

1. **证据门槛**（`MIN_SCORABLE_WORDS = 3`）：可查内容词 <3 的句对不参与 AUC / 报警 /
   排序——之前主动采样 top-10 全是这类短对白，它们不是「可疑」而是「没法评」。
   V6 之前的金标准 91 条里剔掉 31 条薄样本后，**AUC 0.802 → 0.901**。
2. **繁→简归一**：CC-CEDICT 候选是简体、评估语料（和合本、朱譯魔戒）是繁体；用
   对齐器同一张 `TraditionalSimplified` 表归一后，圣经侧 AUC 0.790 → **0.833**，
   同节样本均值 0.11 → 0.15。
3. **大规模验证**（`proxy-scale`，KJV × 和合本逐节真值）。

**当前基线（V6 对齐 + 第 8 轮 bless 后）**：

| 验证集 | 样本 | AUC（够证据） |
| --- | --- | --- |
| 魔戒金标准（人工判定） | 59 条（ok/ok2 48、bad 11） | **0.881**（Wilson 下界 0.828） |
| 圣经机械真值（合计） | 2,992 条（同节 2505、错节 487） | **0.821**（John 0.826 / Genesis 0.805 / Proverbs 0.816） |
| 内部长度比（对照） | 89 条 | 0.731 |
| 内部锚点重叠（对照） | 89 条 | 0.447（反向） |

（V6 把对齐修好之后，剩下的错节更少更难，绝对值从 0.90/0.83 回落到 0.88/0.82，
内部对照同步回落到 0.73/0.45——分离度仍在。）

**P3 候选词特异性加权：评估后不采用（2026-09-08）**。变体对照（**共同子集** =
所有变体都能评分的样本，口径才可比）：
...
- 唯一稳定小胜的是「放宽候选上限 + 特异性加权」（+1.3 / +0.5 点，约 **1 个标准误**），
  且多评分 4% 的句对；但新增样本区分度低（金标准**全集** AUC 被稀释到 0.799），
  报警口径反而变差。
- 单释义、候选 ≥2 字都明显变差——多义噪声和单字候选都带着真信号。
- **结论：收益在噪声量级，不采用**。对照台留在两个工具里（`proxy` 与 `proxy-scale`
  每次都会打印这张表），将来调参一键复跑。

**结论与纪律**：

- 外部信号的**排序能力跨文体稳定**（0.82–0.89），可以决定「谁先被人看」。
- **绝对阈值不跨书**：圣经侧错节基率 16%，t=0.05 时精确率 0.456 / 召回 0.538；
  魔戒侧 t=0.2 精确率 0.58 / 召回 0.92。报警必须**按书内分布归一**（回归门基线），
  不能拿一本书拟合的绝对阈值去套另一本。
- 落地用途（工具直接输出）：**回归门**——整本句对抽样（`pairs-sample.json`，
  ≤2000 条）分布基线当前为均值 0.339 / 中位 0.333 / p25 0.111 / p75 0.500，
...
| 书 | 整本 C/W（字/词） | 平均英文句长 | 平均中文句长 | **句长比 μ_zh/μ_en** | S_zh/S_en（统一口径） |
| --- | --- | --- | --- | --- | --- |
| John | 1.470 | 18.47 词/句 | 23.27 字/句 | **1.260** | 1.166（1.000） |
| Genesis | 1.360 | 22.31 | 24.31 | **1.089** | 1.248（1.097） |
| Proverbs | 1.314 | 15.90 | 13.97 | **0.879** | 1.495（1.010） |
| 魔戒（现代译本） | 1.776 | — | — | **2.026** | 0.877 |

关键恒等式：`C_zh/W_en = (μ_zh/μ_en) × (S_zh/S_en)`。段级对齐要的是前者，句级对齐
要的是后者——两者在圣经差 15–50%，所以**一个全局常量不可能同时伺候两层**。
另注：中英分句口径不一致（中文切 `；`、英文不切 `;`），按统一口径 `S_zh/S_en`
只剩 1.00–1.10，**「中文句数多」大半是分词口径造成的，不是译文性质**。
...
| 整本密度（段=句） | 0.836 / 0.945 / 0.744 | 0.828 / 0.941 / 0.686 | 0.709 / 0.920 / 0.612 |
| 每章密度（段=句） | 0.857 / 0.963 / 0.772 | 0.845 / 0.961 / 0.728 | 0.769 / 0.946 / 0.673 |
| **两层：段=章密度 / 句=句长比** | **0.858 / 0.964 / 0.779** | **0.845 / 0.960 / 0.742** | **0.768 / 0.945 / 0.729** |
| 句长比（两层都用） | 0.535 / 0.793 / 0.431 | 0.518 / 0.782 / 0.387 | 0.305 / 0.664 / 0.264 |
| 字数 × (S_en/S_zh)^0.25 | 0.871 / 0.977 / 0.796 | 0.850 / 0.957 / 0.722 | 0.636 / 0.873 / 0.590 |

- **句长比只在句级用**：两层方案与「每章密度」精度持平（0.858/0.845/0.768），但
  **覆盖率明显更高**——Proverbs 0.673 → 0.729、未定位句对 88 → 24（Genesis 0.728 →
  0.742，John 0.772 → 0.779）。句级对齐的正确单位是「平均句长比」，段级才是「字/词」。
- **句长比两层都用则显著变差**（0.535/0.518/0.305）：段级（节=整段）匹配需要 `C/W`，
  用句长比等于把中文侧按「句数更多」的错觉整体缩短，DP 反向漂移。
- **带句数修正的插值（^0.25）不稳定**：John/Genesis 精度最高（0.871/0.850），
  Proverbs 掉到 0.636 → 不采用。
- **风险在魔戒侧**：魔戒基准读法 45 英文章对 29 中文章，章密度因误配波动到
  0.40–10.31；且它的句长比 2.026 远高于词级密度 1.776（句子比 0.877）。只改段级
  会让 21.5% 句对变化（1.71 已 5.2%、1.75 16.3%），两层都改则 **46.7%**。
  **任何 scale 改动都要重跑金标准，大概率一轮人工判定**；反过来说，魔戒句级
  scale 本来就被 1.7 低估了 16%，「同段句序偏移」那批 bad 样本可能正源于此。

**实现（选项 A，用户 2026-09-08 决定；`TranslationAligner` VERSION 5→6）**：

1. `align()` 先算整本字/词密度 `s_book = ΣC_zh / ΣW_en`（只对全书求和，不依赖章配对），
   供章节对齐与章级估计兜底；
2. 每个已配对章算两级比例：段级 `s_p = clamp(C_zh/W_en)`、句级
   `s_s = clamp((C_zh/S_zh)/(W_en/S_en))`，都钳制 `[0.9, 2.6]`；
3. 段级 DP 用 `s_p`，句级 DP + V5 落盘门槛 + 1:N 合并门槛用 `s_s`；段落兜底的段级项用 `s_p`；
4. **短章护栏**：章内英文词数 < 200 时退回整本密度（`MIN_ADAPTIVE_WORDS`）——几十个词算出的
   密度是噪声（单元测试、封面/标题页），真实章节远高于此；全局常量 1.7 只在整本也拿不到
   特征时兜底；
5. 句数用**终止符近似计数**（`[.!?…]+` / `[。！？；!?;]+`）而非真的分句：密度只要比例，
   实测指标与精确分句持平或略好，省掉「整体先分句一遍、DP 里再分一遍」的双倍开销
   （基准 1408ms → 950ms；V5 基线 514ms）。

**生产实测**（`run-tool.sh generalization`，同书同真值）：

| 书 | V5（全局 1.7） | V6（两级自适应） |
| --- | --- | --- |
...
## 基线（2026-09-26，V7）—— **唯一权威口径**

2026-09-25 在本机从零重建 fixture，2026-09-26 完成 bless。**此后一切判据以本节为准**
（更早那版口径已退役，见下方「历史口径（已退役）」）。

| 阶段 | 句对 / 句级 | 与 `alignment-eval6.csv`（2026-09-03 V5 展示）对账 |
| --- | --- | --- |
| V6（`8c02baa` 之前） | 11,005 / 10,342 | 一致 69 / 差异 25 |
| V6 + 查询侧第 3 级护栏（`8c02baa`） | 11,005 / 10,342（护栏不碰对齐器） | 一致 52 / 差异 42（新增 17 条「句级→段级」） |
| **V7（`b1c24f6`，禁英方并合）** | **10,870 / 10,155** | 一致 58 / **差异 36** |

- **V7 的差异反而少了 6 条**：英方并合下线后，8 条当年靠并合句对「冒充句级」的差异
  （s5/s20/s21/s28/s34/s48/s55/s61）**彻底回到与 eval6 一致的句级**；另有 s86 由段级回句级
  （内容仍与 eval6 不同）。代价是 2 条句级内容变化（s25/s44）。
- **基线**：bless 后 `approved` = **94 条**（66 契约 + 28 已定位 bad），逐字段比对只有
  `approved` 一个新字段；不带 flag 的校验重放 `failures=0`，契约样本 66 保持 / 0 变化，
  bad/skip 28 保持 / 0 变化。**此后任何对齐器改动都要重走：重放 → 人工判定变化样本 →
  `verdict-overrides.json` → fixture 工具 → bless → 校验。**

### 与 eval6 差异的归因（V7 口径，差异 36 条）
...
2. **14 条局部句级/段级落盘差异**——s19 / s43 / s57 / s62 / s63 / s69 / s71 / s74 / s77 /
   s78 / s79 / s80 / s97 / s99：旧重放与当前生产路径在个别段落上一个落句级、一个落段级。
   **其中仅 8 条可确证由查询侧护栏引入**（s1/s2/s7/s24/s27/s31/s32/s33），其余在护栏之前
   就已是段级差异，不能整批归给 `8c02baa`。**s55 在 V7 后已与 eval6 一致，不再属于差异。**
   这批里有 5 条是 `ok2`（s57/s62/s69/s71/s74），当年判定看的是 09-03 的展示，
   **没有按当前展示重新判定过**——下一轮人工判定应优先看这批。
3. **其余未细分**：含 V7 引入的 2 条句级内容变化（s25/s44）与若干 MISS / 段级形态。
   要细分需要插桩对齐器逐条区分「英方并合 vs 中文侧并合 vs 段落兜底」，本轮未做。

## 历史口径（已退役，勿作判据）

2026-09-08 曾在**存档机**上记录过一版数字：11,017 句对（句级 10,323）、契约样本 65 条、
与 eval6 对账 一致 81 / 差异 13。

那批素材（fixture、`pairs-sample.json` 等）只存在于存档机，**现已丢失**：既无法复现，
也无法与本机口径逐条对账（本机同一本书、同一份语料实测为 11,005 / 一致 69 / 差异 25）。
因此这版数字**自 2026-09-26 起退役**——不再作为判据、也不在别处引用；
需要它们请查本文件的 git 历史（2026-09-26 之前的版本）。

`VALIDATION.md` 里 2026-09-08 ~ 09-13 的条目仍会引用这版数字——那是当日的**历史记录**，
保留原样不改写；**读到的任何基线数字一律以本文件「基线（2026-09-26，V7）」为准**。
