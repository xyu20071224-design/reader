...
2. **10 处局部句级/段级落盘差异**——s55 / s57 / s62 / s63 / s69 / s71 / s74 / s77 / s97 / s99：
   ...
   应优先看这批。
   ⚠️ **2026-09-26 更新**：**s55 已不再属于差异**——V7 禁英方并合后它回到与 eval6 一致的
   句级展示（见下节）。同一批在本机口径下实为 14 条（s19/s43/s57/s62/s63/s69/s71/s74/
   s77/s78/s79/s80/s97/s99），且其中**多数在查询侧护栏之前就已存在**，不能整批归给
   `8c02baa`；可确证由护栏引入的只有 8 条（s1/s2/s7/s24/s27/s31/s32/s33）。

其余 81 条与当年 V5 展示逐字一致，说明重放管线（app 格式书 → `TtsTextExtractor`
→ `TranslationAligner` → `TranslationMemoryIndex.lookup`）忠实于历史结论。

## 2026-09-26 本机重新基线（V7，魔戒 100 样本）

> ⚠️ **口径提醒（重要）**：上面「首次基线（2026-09-08）」里的 **11,017 句对 / 契约 65 条 /
> 一致 81 差异 13** 是**存档机**当年的记录。2026-09-25 在另一台机器上从零重建 fixture 后，
> 同一本书、同一份语料实测为 **11,005 句对（句级 10,342）、对账 一致 69 / 差异 25** ——
> 与 11,017/差异 13 的差额（12 条）**至今未归因**，两组数字**不是同一口径，不要混读**。
> （`stage1-before-diffs.txt` 只有 25 行，与「差异 13」对不上；该文件的 mtime 反而晚于
> after 文件，成因未查明。）
...
- **基线（本机）**：bless 后 `approved` = **94 条**（66 契约 + 28 已定位 bad），逐字段比对
  只有 `approved` 一个新字段；不带 flag 的校验重放 `failures=0`，契约样本 66 保持 / 0 变化，
