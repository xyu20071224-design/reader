...
/**
 * 书库仓库核心（桌面迁移 M2 刀7 入 :shared；构造器收 [AppContext]）。
 * Android 的 `:app` facade 保留 SAF 导入（`importBook(uri)` 走 BookImporter facade
 * 后调 [registerImportedBook]）；桌面直接用本类 + File 级导入器。
 */
class LibraryRepository(private val appContext: AppContext) : BookScopedStore {
    ...
    private val legacyMetadataFile = File(appContext.filesDir, "library.json")
    ...
    /** 导入器产出 Book 后的登记口（Android facade 的 importBook 与桌面共用）。 */
    suspend fun registerImportedBook(book: Book): Book = withContext(Dispatchers.IO) {
        mutex.withLock { writeMetadata(book) }
        book
    }
