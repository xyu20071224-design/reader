...
object SentenceSplitter {
    ...
    /**
     * 只数句数、不产出子串（第四轮审查 2-4）。
     *
     * 译本对齐的章级「句级密度」只要一个句数。**必须与 [split] 同规则**：先前
     * `TranslationAligner` 自己数终止符游程，把「引号内的 `？/！` 不算句界」「省略号规则」
     * 「缩略语保护」这些只存在于本类的规则漏掉了，金标准语料实测英文过计 1.65%。
     * 这里**委托** [split]（默认 `maxSentenceLength`），不复制任何规则，口径天然一致。
     */
    fun count(raw: String): Int = split(raw).size
