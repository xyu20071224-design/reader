...
class TextImporter(private val booksDir: File) {
    /** [source] 由调用方负责生命周期（Android facade 传临时拷贝并自删；桌面传原文件）。 */
    fun import(source: File, title: String): Book {
        source.let { require(it.isFile && it.length() > 0) { "文件内容为空" } }
...
fun decodeTextFile(file: File): String {
