...
STATUS = [
 ...
 ("Q1-t01","bug","部分实现","09-26 更新：c7e20de 档案版本闸门（尾部快读判定 isOutdated）+ 书架『重新对齐』入口，让 V3–V7 的修复对**已有档案**可达（此前只影响新对齐的书）。残留：低置信/长度比不合格的句对仍按 V4/V5 设计不落盘（TranslationAligner.kt:194-241），这些句子只有段级兜底；仍不自动重对齐。真机待确认"),
 ...
 ("Q1-t04","bug","已修复","09-26：8c02baa 查询第 3 级同粒度护栏（isCrossSentenceFragment，对老档案立即生效）+ b1c24f6 对齐器 v7 禁英方并合（2 en:1 zh 下线，主路径与邻近兜底一致）→ 点 B 句不再拿到 A 句意思。被并掉的第二句英文不再有句级条目、改走第 5 级段落兜底，属有意降级。实测句对 11005→10870、句级 10342→10155；与 eval6 对账 差异 42→36。真机待确认"),
 ...
 ("Q1-t09","bug","已修复","09-26：c4708fe 四处——①候选与释义面板同源（真实 WordLookup 句子/段落/偏移 + 词典包来源）；②语境优选义项接线 WordAligner.prefer（加成 0.30，只进排序分、不进 confidence，避免重新打开 BUG-039 位置护栏）；③阈值按候选长度分档（1/2/≥3 字 = 0.40/0.72/0.68，每档可拒绝）+ 先过闸再排序；④新增 WordAlignment.sourceSense，面板能把高亮词与来源义项互相指认（新增文案 reader_sense_aligned）。真机待确认"),
